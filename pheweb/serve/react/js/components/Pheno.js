...
class Pheno extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      columns:
        window.browser in phenoTableCols
          ? phenoTableCols[window.browser]
          : null,
    ...
    fetch(`/api/manhattan/pheno/${phenocode}`)
      ...
      .then((data) => {
        ...
        data.unbinned_variants
          .filter((variant) => !!variant.annotation)
          .forEach((variant) => {
            variant.most_severe = variant.annotation.most_severe
              ? variant.annotation.most_severe
                  .replace(/_/g, " ")
                  .replace(" variant", "")
              : "";
        ...
        data.unbinned_variants.forEach((variant) => {
          if (variant.pval == 0) variant.pval = pval_sentinel;
        });
        //TODO server side
        data.unbinned_variants.forEach((variant) => {
          ...
          if (!variant.gnomad) {
            ...
            +variant.gnomad["AC_nfe_nwe"] +
              +variant.gnomad["AC_nfe_onf"] +
              +variant.gnomad["AC_nfe_seu"] ==
            0
          ) {
            variant.fin_enrichment = 1e6;
          } else {
            variant.fin_enrichment =
              +variant.gnomad["AC_fin"] /
              +variant.gnomad["AN_fin"] /
              ((+variant.gnomad["AC_nfe_nwe"] +
                +variant.gnomad["AC_nfe_onf"] +
                +variant.gnomad["AC_nfe_seu"]) /
                (+variant.gnomad["AN_nfe_nwe"] +
                  +variant.gnomad["AN_nfe_onf"] +
                  +variant.gnomad["AN_nfe_seu"]));
          }
        });
  ...
  getQQ(phenocode) {
    fetch(`/api/qq/pheno/${phenocode}`)
      ...
      .then((data) => {
        ...
        if (data.by_maf) create_qq_plot(data.by_maf);
        else
          create_qq_plot([
            {
              maf_range: [0, 1],
  ...
  download() {
    const data =
      this.state.selectedTab == 0
        ? this.cstable.getResolvedState().sortedData.map((datum) =>
            Object.keys(datum).reduce((acc, cur) => {
              if (!cur.startsWith("_")) acc[cur] = datum[cur];
              return acc;
            }, {})
          )
        : this.vartable.getResolvedState().sortedData.map((datum) =>
            Object.keys(datum).reduce((acc, cur) => {
              if (!cur.startsWith("_")) acc[cur] = datum[cur];
              return acc;
            }, {})
  ...
  render() {
    ...
    const ukbb =
      window.show_ukbb == "True" ? (
        this.state.ukbb_n ? (
          <div>
            UKBB: <strong>{this.state.ukbb_n[0]}</strong> cases,{" "}
            <strong>{this.state.ukbb_n[1]}</strong> controls
          </div>
        ) : (
    ...
    const n_cc1 = pheno.cohorts ? (
      <tbody>
        ...
        <tr>
          <td>
            <b>
              {pheno.cohorts.reduce((acc, cur) => acc + cur.num_controls, 0)}
            </b>{" "}
            controls
          </td>
        </tr>
      ...
      <tbody>
        <tr>
          <td>
            <b>{pheno.num_cases}</b> cases
          </td>
        </tr>
        <tr>
          <td>
            <b>{pheno.num_controls}</b> controls
          </td>
        </tr>
      </tbody>
    ) : pheno.num_samples ? (
    ...
    const n_cc2 = pheno.cohorts ? (
      <div>
        ...
        <table className="column_spacing">
          <tbody>
            {pheno.cohorts
              .sort((c1, c2) => c1.cohort.localeCompare(c2.cohort))
              .map((cohort) => (
                <tr key={cohort.cohort}>
                  <td>{cohort.cohort}</td>
                  <td>
                    <b>{cohort.num_cases}</b> cases
                  </td>
                  <td>
                    <b>{cohort.num_controls}</b> controls
                  </td>
                </tr>
              ))}
          </tbody>
      ...
      <div>
        <ReactTable
          ...
          SubComponent={(row) => (
            <ReactTable
              data={
                this.state.locus_groups.hasOwnProperty(
                  row["original"]["locus_id"]
                )
                  ? this.state.locus_groups[row["original"]["locus_id"]]
                  : (this.getGroup(
                      this.state.phenocode,
                      row["original"]["locus_id"]
                    ),
                    this.state.locus_groups[row["original"]["locus_id"]])
              }
        ...
        <div className="row">
          <div className="col-xs-12">
            <div className="btn btn-primary" onClick={this.download}>
              Download table
            </div>
          </div>
        </div>
        <CSVLink
          ...
          filename={
            this.state.selectedTab == 0
              ? `finngen_${window.release}_${pheno.phenocode}_autorep.tsv`
              : `finngen_${window.release}_${pheno.phenocode}_lead.tsv`
          }
          className="hidden"
    ...
    const qq_table = this.state.qq ? (
      <div>
        <table className="column_spacing">
          <tbody>
            {Object.keys(this.state.qq.overall.gc_lambda)
              .sort()
              .reverse()
              .map((perc) => (
                <tr key={perc}>
    ...
    const tabPanel =
      this.state.columns.length != 0 && this.state.csColumns.length != 0 ? (
        <Tabs
    ...
    return (
      <div style={{ width: "100%", padding: "0" }}>
        ...
        {risteys}
        <table className="column_spacing">{n_cc1}</table>
        {ukbb}
