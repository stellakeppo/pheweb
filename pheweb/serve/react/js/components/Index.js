...
class Index extends React.Component {
  constructor(props) {
    ...
    this.state = {
      phenolistColumns:
        window.browser in phenolistTableCols
          ? phenolistTableCols[window.browser]
          : null,
      filtered: [],
      dataToDownload: [],
    };
  ...
  getConfig() {
    fetch("/api/config/ui")
      ...
      .then((_) => {
        (this.state && this.state.phenolistColumns) ||
  ...
  render() {
          ...
          <div className="col-xs-12">
            <div className="btn btn-primary" onClick={this.download}>
              ...
              {this.state.filtered.filter(
                (f) =>
                  !(f.id == "variant_category" && f.value == "all") &&
                  !(f.id == "variant" && f.value == "all")
              ).length > 0
                ? "filtered"
                : ""}{" "}
              table
    ...
    return <div style={{ width: "100%", padding: "0" }}>{phenoTable}</div>;
  }
}
