...
def test_all(tmpdir, capsys):
    ...
    cl_run(
        conf
        + ["phenolist", "glob", "--simple-phenocode", "{}/assoc-files/*".format(indir)]
    )
    ...
    cl_run(
        ...
        + [
            ...
            "import-phenolist",
            "-f",
            "{}/pheno-list-categories.json".format(p),
            "{}/categories.csv".format(indir),
        ]
    )
    cl_run(
        conf + ["phenolist", "merge-in-info", "{}/pheno-list-categories.json".format(p)]
    )
