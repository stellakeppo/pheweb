...
task preprocess {
     ...
     command <<<
	   ...
	   cat "${summary_file}" | \
	   (if [[ "${summary_file}" == *.gz || "${summary_file}" == *.bgz ]]; then zcat ; else cat ; fi) | \
	   ${default="cat" preprocessor } | \
	   awk -v FS=$'\t' -v OFS='\t' 'NR > 1 { $2 = int($2); } 1' | \
	   awk -v FS=$'\t' -v OFS='\t' 'BEGIN  { x["X"]=23; x["Y"]=24; x["M"]=25;x["MT"]=25 } { $1 = ($1 in x? x[$1] : $1) } 1' | \
	   sort -t$'\t' -k1,1n -k2,2n -k3,3 -k4,4 | bgzip > "${dir}${out_filename}"
...
task sites {
           Array[File] summary_files
     	   String docker
           Int disk
	   # post the pre-processing task it is assumed the file
	   # is of form chrom, pos, ref, alt ...
	   # chrom : 1..25
	   # pos   : N
	   # ref   : [GATC]+
	   # ref   : [GATC]+
...
task matrix {
    ...
    command <<<
        ...
        def multiproc(i):
            ...
            print(file)
            cmd = ["external_matrix.py", file, file + ".", "${sites}.noheader", "--chr", "#chrom", "--pos", "pos", "--ref", "ref", "--alt", "alt", "--no_require_match", "--no_tabix"]
            start = time.time()
